...
impl TryFrom<Expression> for Expr {
    ...
    fn try_from(expr: Expression) -> Result<Self, Self::Error> {
        match *expr.0 {
            ...
            SqlExpr::Wildcard(_) => Ok(Self::Wildcard),
            // SqlExpr::IsNull(expr) => Ok(Self::IsNull(Arc::new(Expression(expr).try_into()?))),
            // SqlExpr::IsNotNull(expr) => Ok(Self::IsNotNull(Arc::new(Expression(expr).try_into()?))),
            SqlExpr::Identifier(id) => Ok(Self::Column(PlSmallStr::from(id.value))),
